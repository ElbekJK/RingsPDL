...
Download ringpy.py and place it in your project directory.
Usage
...
E_wg, T_wg = ringpy.compute_transmission(
    case='waveguide',
    wavelengths=wl,
    layers=layers,
    T=30,
...
ringpy.plot_transmission(wl, T_wg, labels=['Waveguide'])
...
(_, _), (T_thru, T_drop) = ringpy.compute_transmission(
    case='mrr_add_drop',
    wavelengths=wl,
...
ringpy.plot_transmission(wl, T_thru, T_drop, labels=['Elliptical Through', 'Elliptical Drop'])
...
_, T_multi, drops = ringpy.compute_transmission(
    case='multi_mrr',
    wavelengths=wl,
    params_list=params_list,
    bus_segments=bus_segments
)
ringpy.plot_transmission(wl, T_multi, drops, labels=['Cascaded Through', 'Drop 1', 'Drop 2'])

Example 4: Mode Profile and Group Index
E_field, n_eff = ringpy.solve_waveguide_modes(wl, layers, T=30, polarization='TE', num_modes=2)
ringpy.plot_mode_profile(E_field[500], layers, labels=['Mode 0', 'Mode 1'], wl=wl[500])
n_g = ringpy.compute_group_index(wl, n_eff[:, 0])  # Group index for fundamental mode
plt.plot(wl * 1e9, n_g)
...
Equation: For TE (( E_y )):[\frac{d^2 E_y}{dx^2} + \left( k_0^2 n^2(x) - \beta^2 \right) E_y = 0]With PML: Complex coordinate stretching.
...
Equations:[H_{\text{thru}} = \frac{t_1 - t_2 A e^{-j \phi}}{1 - t_1 t_2 A e^{-j \phi}}, \quadH_{\text{drop}} = -\kappa_1 \kappa_2 \sqrt{A} e^{-j \phi / 2} / (1 - t_1 t_2 A e^{-j \phi})]
